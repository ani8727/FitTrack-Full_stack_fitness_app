...
/**
 * Custom Health Check for Admin Service Database Connection
 * Validates that the admin_service_user role can access admin_db exclusively
 */
@Slf4j
@Component("adminDatabaseHealth")
public class DatabaseHealthIndicator implements HealthIndicator {
    ...
    private Health checkDatabaseHealth() {
        try (Connection connection = dataSource.getConnection();
            ...
            if (!"admin_db".equals(currentDatabase)) {
                log.warn("⚠ Connected to database: {} (expected: admin_db)", currentDatabase);
            }

            // Validate role-based access
            if (!currentUser.contains("admin_service_user")) {
                log.warn("⚠ Connected as user: {} (expected: admin_service_user)", currentUser);
            }
