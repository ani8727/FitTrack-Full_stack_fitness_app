...
#### 5. Create Initial Admin (Keycloak-managed)
...
4. Go to **Credentials** tab → **Set password**
   - Password: `admin123!`
   - Temporary: **OFF**
5. Go to **Role mapping** tab → **Assign role** → choose `ADMIN` and `USER`
...
4. Go to **Credentials** tab → **Set password**
   - Password: `test123`
   - Temporary: **OFF**
5. Go to **Role mapping** tab → **Assign role** → choose `USER`
...
- Click **Create Account** in the app to self-register; created accounts receive the `USER` role.
- Email/password are stored in Keycloak; the userservice mirrors the account for profile data.

### Login (Keycloak SSO)

- Click **Sign In** in the app; you will be redirected to Keycloak.
- Use any Keycloak user with the `USER` role (e.g., `testuser` / `test123`).
- Admins sign in with Keycloak accounts that have the `ADMIN` role (e.g., `admin` / `admin123!`).
- Self-service registration in the app creates `USER` accounts only; admin accounts must be provisioned in Keycloak.
...
### Admin Features (Keycloak-provisioned only)

- Admin Dashboard and Admin Users pages require an `ADMIN` role in the Keycloak token.
- Typical tasks: view user list, filter/search, update user status (e.g., suspend/reactivate), and view user stats.
- Admin accounts must be created/assigned in Keycloak; there is no in-app admin registration form.

### Profile & Account Settings

- Access **Profile** to view your data; access **Settings → Account** to update profile details.
- Users can update name, email, goals, and other profile fields for their own account only.
- Authorization is enforced server-side: users can edit only their own profile; admins may manage others where allowed by the API.
